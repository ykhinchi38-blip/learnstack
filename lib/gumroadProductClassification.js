...
function collectCandidates(product, fields) {
  return fields
    .map((field) => {
      const raw = normalizeValue(field.includes(".") ? readNestedValue(product, field) : product[field]);
...
export function isBundleProduct(product = {}) {
  const primaryCandidates = collectCandidates(product, BUNDLE_PRIMARY_FIELDS);
  const primaryMatch = primaryCandidates.some((candidate) => candidate.variants.some((variant) => variant.includes("learnstackbundle")));

  if (primaryMatch) {
    return true;
  }

  const titleCandidates = collectCandidates(product, ["name", "title"]);
  const titleMatch = titleCandidates.some((candidate) => /\bbundles?\b/i.test(candidate.raw));

  if (titleMatch) {
    return true;
  }

  const description = normalizeValue(product.description || product.raw?.description);
  return /\bbundles?\b/i.test(description);
}
