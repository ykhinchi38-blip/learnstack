...
import { formatPrice, getProductPricing } from "./pricing";

const TEN_MINUTES = 600;
const PRODUCTS_PER_PAGE = 100;
const MAX_PRODUCT_PAGES = 50;
const GUMROAD_REQUEST_TIMEOUT_MS = 10_000;
...
async function gumroadFetch(path) {
  ...
  const response = await new Promise((resolve, reject) => {
    timeoutId = setTimeout(() => {
      controller.abort();
      reject(new Error(`Gumroad API request timed out after ${GUMROAD_REQUEST_TIMEOUT_MS / 1000} seconds.`));
    }, GUMROAD_REQUEST_TIMEOUT_MS);

    fetch(gumroadUrl(path), {
...
function previewImageResult(product, coverImage) {
  ...
  return {
    urls: getUniqueImages(images, [coverImage]),
...
export function normalizeGumroadProduct(product = {}) {
  ...
  const currency = product.currency || product.currency_type || "USD";
  ...
  const pricing = getProductPricing({ title: name, slug, price: priceAmount, priceCents, currency });
  ...
  return {
    ...
    includedProductIds: product.included_product_ids || product.includedProductIds || [],
    includedProducts: product.included_products || product.includedProducts || [],
    products: product.bundle_products || product.bundleProducts || product.products || [],
    raw: {
...
function getDiscount(offer = {}, product = {}) {
  ...
  if (amountCents > 0 && productPriceCents > 0) {
    ...
    return {
      ...
      discountAmount: `${formatPrice(amountCents / 100, "USD")} off`
    };
  }
