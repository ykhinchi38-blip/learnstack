...
export function getPublicSamples(products = [], samples = sampleManifest) {
  ...
  (Array.isArray(products) ? products : []).forEach((product) => {
    ...
    if (!match.sample || !hasValidSampleUrl(match.sample)) return;

    if (key) seenProducts.add(key);
    matchedSampleSlugs.add(match.sample.sourceSlug);
    ...
    publicSamples.push({
      ...
      sample: match.sample,
      sampleUrl: match.sample.sampleUrl,
      category: match.sample.category,
      sourceSlug: match.sample.sourceSlug,
      matchSlug: match.sample.matchSlug,
      title: product.title || match.sample.title,
      description: product.summary || product.tagline || product.limitedDescription || product.description || "",
