...
import { isBundleProduct } from "@/lib/gumroadProductClassification";
import { getProductPricing } from "@/lib/pricing";
import { getProductOutcomes } from "@/data/productOutcomes";
...
function isPublishedProduct(product = {}) {
  return String(product.status || "published").toLowerCase() === "published" && product.published !== false;
}
...
function productDetailPath(product) {
  const basePath = product.audience === "kids"
    ...
    : product.audience === "life-career"
      ? "/life-career"
      : product.audience === "bundle" || product.isBundle
        ? "/bundles"
        : "/products";
...
function mapGumroadProduct(product) {
  ...
  const audience = isBundle ? "bundle" : product.audience || "regular";
    ...
    : audience === "life-career"
      ...
      : audience === "business"
        ...
        : audience === "bundle"
          ? "Bundles"
        : "New Releases";
  const defaultAccent = audience === "kids" ? "#FF8FAB" : audience === "life-career" ? "#1a3a7c" : audience === "bundle" ? "#ffd34d" : "#2563EB";
  ...
  const mappedProduct = {
    ...
    priceVerification: pricing.priceVerification,
    priceDisplay: pricing.priceDisplay,
    productType: product.productType || (audience === "kids" ? "kids-book" : isBundle ? "bundle" : "handbook"),
    status: product.status || (product.published === false ? "draft" : "published"),
    featured: Boolean(product.featured),
    relatedProductIds: product.relatedProductIds || [],
    includedProductIds: product.includedProductIds || product.included_product_ids || product.raw?.includedProductIds || product.raw?.included_product_ids || [],
    includedProducts: product.includedProducts || product.included_products || product.raw?.includedProducts || product.raw?.included_products || [],
    products: product.products || product.bundleProducts || product.bundle_products || product.raw?.products || product.raw?.bundleProducts || product.raw?.bundle_products || [],
    legacyPrice: product.legacyPrice || null,
    legacyCurrency: product.legacyCurrency || null,
    image: coverImage,
    coverImage,
    ...
    gumroadUrl,
    url: gumroadUrl,
    permalink: product.permalink || product.raw?.permalink || null,
    customPermalink: product.customPermalink || product.custom_permalink || product.raw?.custom_permalink || null,
    linkName: product.linkName || product.link_name || product.raw?.linkName || product.raw?.link_name || null,
    accentColor: product.accentColor || defaultAccent,
    logoText: product.logoText || (audience === "kids" ? "KID" : audience === "bundle" ? "BND" : shortTitle(title)),
    topics: product.topics || (audience === "kids"
      ? ["Kids", "PDF", "Activities", "Instant Download"]
      : audience === "life-career"
        ? ["Life Skills", "Career", "Self-study", "Instant Download"]
        : audience === "bundle"
          ? ["Bundle", "Digital Books", "Gumroad", "Learning Pack"]
          : ["PDF", "Handbook", "Instant Download"]),
    description,
    limitedDescription,
    longDescription: product.longDescription || description,
    level: product.level || product.skillLevel || null,
    skillLevel: product.skillLevel || product.level || null,
    ...
    bestSuitedFor: product.bestSuitedFor || product.best_suited_for || [],
    prerequisites: product.prerequisites || [],
    whatInside: product.whatInside || [],
    outcomes: Array.isArray(product.outcomes) && product.outcomes.length ? product.outcomes : getProductOutcomes(product),
    learningOutcomes: product.learningOutcomes || product.whatYouWillLearn || [],
    projects: product.projects || [],
    examples: product.examples || [],
    exercises: product.exercises || [],
    cheatSheets: product.cheatSheets || product.cheat_sheets || [],
    activities: product.activities || [],
    coloringPages: product.coloringPages || product.coloring_pages || [],
    discussionPrompts: product.discussionPrompts || product.discussion_prompts || [],
    illustrations: product.illustrations || [],
    illustrated: Boolean(product.illustrated),
    themes: product.themes || [],
    classroomUse: product.classroomUse || null,
    ...
    badge: product.badge || (audience === "kids" ? "Kids" : audience === "bundle" ? "Bundle" : "New"),
...
export async function getProductsByAudienceResult(audience) {
  ...
  const products = result.products.filter((product) => isPublishedProduct(product) && (product.audience || "regular") === audience);
...
export async function getBundleProductsResult() {
  return getProductsByAudienceResult("bundle");
}

export async function getGumroadProducts() {
  const result = await getAllProductsResult();
  return result.products.filter(isPublishedProduct);
}

export async function getAllProducts() {
  const result = await getAllProductsResult();
  const products = result.error || !result.products.length ? localProducts.map(mapGumroadProduct) : result.products;
...
export async function getBundleProducts() {
  return getProductsByAudience("bundle");
}
...
export async function getBundleProductBySlug(slug) {
