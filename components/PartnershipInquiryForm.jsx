...
export default function PartnershipInquiryForm() {
  ...
  const fallbackEmail = process.env.NEXT_PUBLIC_PARTNERSHIP_EMAIL || process.env.NEXT_PUBLIC_SUPPORT_EMAIL || "";
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!response.ok || result.success !== true) {
        ...
        setStatus({
          type: "error",
          message: result.error || "Your request could not be confirmed. Please try again.",
          fallbackEmail: result.fallbackEmail || fallbackEmail,
          contactPath: result.contactPath || ""
        });
        return;
      }
  ...
  return (
    <>
      ...
      {status.type !== "idle" && <output className={`${styles.status} ${styles[status.type]}`} role={status.type === "error" ? "alert" : "status"}>{status.message}{status.fallbackEmail && <> <a href={`mailto:${status.fallbackEmail}`}>{status.fallbackEmail}</a>.</>}{status.contactPath && <> <a href={status.contactPath}>Contact LearnStack</a>.</>}</output>}
      ...
      <PartnershipSuccessModal open={successOpen} onClose={() => setSuccessOpen(false)} />
    </>
  );
}
