...
function localFallbackProducts(audience) {
  const published = localProducts.filter((product) => String(product.status || "published").toLowerCase() === "published");
  if (audience === "all") return published;
  return published.filter((product) => (product.audience || "regular") === audience);
}
