...
function validate(values) {
  ...
  ["website", "linkedinProfile", "otherProfileLink", "sampleContentLink"].forEach((field) => {
    if (values[field] && !/^https?:\/\//i.test(values[field])) errors[field] = "Use a full URL starting with http:// or https://.";
  });
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit} onFocus={trackFormStart} noValidate>
      <div className={styles.formGrid}>
        ...
        <label>Website<input type="url" value={values.website} onChange={(event) => update("website", event.target.value)} aria-invalid={Boolean(errors.website)} placeholder="https://" /></label>
        <label>LinkedIn profile<input type="url" value={values.linkedinProfile} onChange={(event) => update("linkedinProfile", event.target.value)} aria-invalid={Boolean(errors.linkedinProfile)} placeholder="https://" /></label>
        <label>Other profile link<input type="url" value={values.otherProfileLink} onChange={(event) => update("otherProfileLink", event.target.value)} aria-invalid={Boolean(errors.otherProfileLink)} placeholder="https://" /></label>
